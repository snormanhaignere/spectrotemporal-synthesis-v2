...
function [filts,Hz_cutoffs,freqs] = make_erb_cos_filts_double2(signal_length, sr, N, low_lim, hi_lim, animal)
...
for k=1:num_filters
    l = erb2freq_fun(center_freqs(k)-2*spacing);
    h = erb2freq_fun(center_freqs(k)+2*spacing);
...
center_freqs = erb2freq_fun([center_freqs(1)-2*spacing center_freqs(2)-2*spacing center_freqs center_freqs(num_filters-1)+2*spacing center_freqs(num_filters)+2*spacing]);
